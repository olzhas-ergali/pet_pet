      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-4">
        ...
        <dl className="space-y-3 text-sm">
          <div className="flex justify-between gap-4 border-b border-gray-100 pb-3">
            <dt className="text-gray-500">{t('profile.fieldPhone')}</dt>
            <dd className="font-medium text-gray-900 text-right">
              {user?.phone ?? '—'}
            </dd>
          </div>
          <div className="flex justify-between gap-4 border-b border-gray-100 pb-3">
            <dt className="text-gray-500">{t('profile.fieldEmail')}</dt>
            <dd className="font-medium text-gray-900 text-right break-all">
              {user?.email ?? '—'}
            </dd>
          </div>
          <div className="flex justify-between gap-4">
            <dt className="text-gray-500">{t('profile.fieldDisplayName')}</dt>
            <dd className="font-medium text-gray-900 text-right">
              {user?.displayName ?? '—'}
            </dd>
          </div>
